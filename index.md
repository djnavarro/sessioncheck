...
at the start of a script), the package also supplies a
...
The
[`sessionstate()`](https://sessioncheck.djnavarro.net/reference/sessionstate.md)
function is similar in spirit to session information functions provided
by base R and other packages. For details on the similarities and
differences between them, and when to prefer one over the other see the
[session state
