...
script from running, and prompt the human user to consider restarting
the R session.

To illustrate what happens in a contaminated R session, I’ll do a few
things that most R users would agree fit the definition of a
“contaminated” R environment: creating objects in the global
...
These actions vary considerably in their riskiness. For the example
above, here’s a quick risk assessment:

- **Attaching packages**. It’s possible that attaching the
  **sessioncheck** package with
  [`library()`](https://rdrr.io/r/base/library.html) might cause
  problems, but the risk varies a lot depending on what the package
  does. I’ve tried to design **sessioncheck** safely, so most likely
  this specific command won’t be a problem. But that won’t always be
  true, and therefore your script should notify you about any unexpected
  packages that it detects. In any case, for the most part you wouldn’t
  bother attaching this package. The typical use case for this package
  is to call
  [`sessioncheck::sessioncheck()`](https://sessioncheck.djnavarro.net/reference/sessioncheck.md)
  once, at the top of the script. In the normal course of events you
  wouldn’t be writing
  [`library(sessioncheck)`](https://github.com/djnavarro/sessioncheck)
  in your script, so yes, the user should probably be informed. It’s
  just politeness!

- **Adding objects to the global environment**. Adding the `my_data`
  data frame as a variable in the global environment is not in itself a
  bad thing, but in practice when you’re running the same script over
  and over in an interactive context – which is a thing that data
  analysts have to do a lot, much more so than software engineers – you
  can quickly build up a lot of “detritus”, bits and pieces and objects
  left behind from code you tried out previously. Over time it becomes a
  big problem, because it’s harder for you to reason about what’s going
  on in your data. The user should definitely be informed, because there
  are real risks here.
...
#> ✖ Unexpected objects in global environment: my_data
#> ✖ Unexpected packages: sessioncheck
#> ✖ Unexpected environments attached: my_data
```
