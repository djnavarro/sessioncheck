...
The default behaviour is to warn the user if issues are detected, but this can be converted to an error by using `sessioncheck::sessioncheck("error")` if a stricter check is required.
