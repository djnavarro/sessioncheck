...
format.sessioncheck_sessionstate <- function(x, platform = NULL, locale = NULL, matrix = NULL, document = NULL, machine = NULL, git = NULL, timing = NULL, rng = NULL, packages = NULL, globalenv = NULL, globalenv_n = NULL, attachments = NULL, ...) {
  ...
  platform_all <- c(
    version = .bullet_line(sprintf("%-20s%s", "version", p$version)),
    os      = .bullet_line(sprintf("%-20s%s", "os", p$os)),
    system  = .bullet_line(sprintf("%-20s%s", "system", p$system)),
    ui      = .bullet_line(sprintf("%-20s%s", "ui", p$ui)),
    tz      = .bullet_line(sprintf("%-20s%s", "tz", p$tz)),
    date    = .bullet_line(sprintf("%-20s%s", "date", p$date))
  )
  ...
  locale_all <- c(
    language = .bullet_line(sprintf("%-20s%s", "language", if (is.na(loc$language)) "(unset)" else loc$language)),
    collate  = .bullet_line(sprintf("%-20s%s", "collate", if (is.na(loc$collate)) "(unknown)" else loc$collate)),
    ctype    = .bullet_line(sprintf("%-20s%s", "ctype", if (is.na(loc$ctype)) "(unknown)" else loc$ctype))
  )
  ...
  matrix_all <- c(
    blas   = .bullet_line(sprintf("%-20s%s", "BLAS", mx$blas)),
    lapack = .bullet_line(sprintf("%-20s%s", "LAPACK", mx$lapack))
  )
  ...
  document_all <- c(
    pandoc = .bullet_line(sprintf("%-20s%s", "pandoc", if (is.na(doc$pandoc)) "(not found)" else doc$pandoc)),
    quarto = .bullet_line(sprintf("%-20s%s", "quarto", if (is.na(doc$quarto)) "(not found)" else doc$quarto))
  )
  ...
  git_all <- c(
    ...
    sha   = .bullet_line(sprintf("%-20s%s", "commit sha", if (is.na(g$sha)) "(not a git repository)" else g$sha)),
    dirty = .bullet_line(sprintf("%-20s%s", "dirty", if (is.na(g$dirty)) "(unknown)" else g$dirty))
  )
  ...
  timing_all <- c(
    captured_at = .bullet_line(sprintf("%-20s%s", "captured at", format(t$captured_at, usetz = TRUE))),
    # "session uptime" (rather than e.g. "session time"/"runtime") to make
    # clear this is how long the R session itself has been alive, not how
    # long the calling script took to run -- the most likely
    # misinterpretation for the primary use case of calling
    # sessionstate() at the end of a script. The unit lives on the value
    # ("... sec") rather than the label, so the label stays short enough
    # to share the same field width as every other section.
    elapsed_sec = .bullet_line(sprintf("%-20s%s", "session uptime", sprintf("%s sec", t$elapsed_sec)))
  )
  ...
  rng_all <- c(
    kind        = .bullet_line(sprintf("%-20s%s", "kind", r$kind)),
    normal_kind = .bullet_line(sprintf("%-20s%s", "normal kind", r$normal_kind)),
    sample_kind = .bullet_line(sprintf("%-20s%s", "sample kind", r$sample_kind)),
    seed_hash   = .bullet_line(sprintf("%-20s%s", "seed hash", if (is.na(r$seed_hash)) "(not set)" else r$seed_hash))
  )
